...
/// <summary>
/// Extension methods for LogRepository providing additional utility operations.
/// </summary>
public static class LogRepositoryExtensions
{
    ...
    public static async Task<Dictionary<SyslogLevel, int>> GetLogLevelCountsAsync(
        ...
        CancellationToken ct = default)
    {
        var allLogs = await repository.GetCountAsync(ct);
        var levelLogs = await repository.GetByLevelAsync(SyslogLevel.Debug, ct);
        var errorLogs = await repository.GetByLevelAsync(SyslogLevel.Error, ct);
        var infoLogs = await repository.GetByLevelAsync(SyslogLevel.Info, ct);
        var warningLogs = await repository.GetByLevelAsync(SyslogLevel.Warning, ct);

        return new Dictionary<SyslogLevel, int>
        {
            [SyslogLevel.Debug] = await repository.GetByLevelAsync(SyslogLevel.Debug, ct).ContinueWith(t => t.Result.Count()),
            [SyslogLevel.Error] = await repository.GetByLevelAsync(SyslogLevel.Error, ct).ContinueWith(t => t.Result.Count()),
            [SyslogLevel.Info] = await repository.GetByLevelAsync(SyslogLevel.Info, ct).ContinueWith(t => t.Result.Count()),
            [SyslogLevel.Warning] = await repository.GetByLevelAsync(SyslogLevel.Warning, ct).ContinueWith(t => t.Result.Count())
        };
    }
